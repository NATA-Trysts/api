	...
	actions: {
		...
		add: {
			...
			async handler(ctx) {
				...
				if (item) throw new MoleculerClientError('User has already followed')
		...
		has: {
			...
			async handler(ctx) {
				const item = await this.findByFollowAndUser(
					ctx.params.follow,
					ctx.params.user
				)
				return !!item
		...
		count: {
			...
			async handler(ctx) {
				let query = {}
				if (ctx.params.follow) query = { follow: ctx.params.follow }

				if (ctx.params.user) query = { user: ctx.params.user }
		...
		delete: {
			...
			async handler(ctx) {
				const { follow, user } = ctx.params
				const item = await this.findByFollowAndUser(follow, user)
				if (!item) throw new MoleculerClientError('User has not followed yet')
